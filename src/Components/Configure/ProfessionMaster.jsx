...
const ProfessionMaster = withStyles(styles)(props => {
  ...
  useEffect(() => {
    fetch('http://206.189.130.102:5000/api/v1/get-proffesion')
      .then(res => res.json())
      .then(data => {
  ...
  const DeleteConfirmation = () => {
    {
      ...
      fetch(`http://206.189.130.102:5000/api/v1/delete-proffesion/${ProfId}`, {
        method: "DELETE"
      }).then((result) => {
        result.json().then((res) => {
          console.warn(res)
          window.alert("Profession removed")
        })
      })
    }
  }
    ...
    <RootStyle>
      ...
      >
        <Box component='form' onSubmit={adddata} sx={style2}>
          ...
          <input type="text" name='proffesionName' value={profdata.proffesionName} onChange={Change} className="editinput" placeholder='New profession' />
          <button className="submitbtn">Submit</button>
        </Box>
      </Modal>

      <div className='content'>
        <h1 className="heading3">Profession Master</h1>
              ...
              : <>
                <TableHead>
                  ...
                  <StyledTableRow>
                    ...
                    <StyledTableCell className="row2" align="left">Profession</StyledTableCell>
                ...
                <TableBody>
                  {profession.map((row, index) => (
                    <StyledTableRow key={index}>

                      <StyledTableCell align="left">&nbsp;&nbsp;{index + 1}</StyledTableCell>
                      <StyledTableCell align="left">{row.proffesionName}</StyledTableCell>

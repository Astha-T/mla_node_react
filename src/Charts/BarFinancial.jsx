...
const BarFinancial = () => {
  ...
  useEffect(() => {
    fetch(url)
      ...
      .then(data => {
        ...
        let updatedList = data.data.find(updatedList => updatedList.finacialData)

        setFinancial(updatedList.finacialData)
        setDataLength(updatedList.finacialData.length)
      })
  })

  const count = {};

  Array.from(financial).forEach(element => {
  ...
  const notsurveyed = financial.filter(r=>r===null)
  ...
  const chartData = chartData2.map((val, index) => {
    return {
      state: val.financial, votes: val.quantity
    }
  })
  ...
  return (
    (Status &&
      <>
        <h1 className="piecharttitle" style={{ marginBottom: '2%', paddingTop: '2%', paddingBottom: '2%' }}>Financial Count</h1>

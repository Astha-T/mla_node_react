...
const BarCaste = () => {
  ...
  useEffect(() => {
    fetch(url)
      ...
      .then(data => {
        ...
        let updatedList = data.data.find(updatedList => updatedList.casteData)

        setCaste(updatedList.casteData)
        setDataLength(updatedList.casteData.length)
      })
  })

  const count = {};

  Array.from(caste).forEach(element => {
  ...
  const notsurveyed = caste.filter(r=>r===null)
  ...
  const chartData = chartData2.map((val, index) => {
    return {
      state: val.caste, votes: val.quantity
    }
  })
  ...
  return (
    (Status &&
      <>
        <h1 className="piecharttitle" style={{ marginBottom: '2%', paddingTop: '2%', paddingBottom: '2%' }}>Caste (SubCaste) Count</h1>

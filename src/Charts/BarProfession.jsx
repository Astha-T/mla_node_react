...
const BarProfession = () => {
  ...
  useEffect(() => {
    fetch(url)
      ...
      .then(data => {
        ...
        let updatedList = data.data.find(updatedList => updatedList.professionData)

        setProfession(updatedList.professionData)
        setDataLength(updatedList.professionData.length)
      })
  })

  const count = {};

  Array.from(profession).forEach(element => {
  ...
  const notsurveyed = profession.filter(r=>r===null)
  ...
  const chartData = chartData2.map((val, index) => {
    return {
      state: val.profession, votes: val.quantity
    }
  })
  ...
  return (
    (Status &&
      <>
        <h1 className="piecharttitle" style={{ marginBottom: '2%', paddingTop: '2%', paddingBottom: '2%' }}>Profession Count</h1>
